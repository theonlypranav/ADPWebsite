...
function Inventory() {
    ...
    const fetchItems = async () => {
      try {
        const response = await fetch(
          'https://adpwebs-a4geehfwhtchdraw.centralindia-01.azurewebsites.net/api/cart/cart-item-summary-custom',
          {
            headers: {
  ...
  return (
    ...
    >
      <h1 className="text-4xl font-bold mb-6 text-center">External Inventory Demand</h1>
  
      <div className="flex align-center justify-center w-full mb-8">
        <Link to="/inventoryadp">
          <button className="bg-blue-500 text-white px-4 py-2 rounded mr-4">Back to Home</button>
        </Link>
      ...
      <div className="overflow-x-auto rounded-lg shadow-lg border border-blue-400 glow w-full">
      ...
      <button onClick={handleSave} className="bg-green-500 text-white px-4 py-2 rounded mt-8">
        Save Changes
      </button>
  
      {modalVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 text-black dark:text-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
            <button
              onClick={closeModal}
